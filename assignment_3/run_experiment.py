...
if __name__ == '__main__':
    ...
    datasets = []
    dataset1_details = {
            'data': loader.Adult(verbose=verbose, seed=seed),
            'name': 'Adult',
            'readable_name': 'Adult Dataset',
            'best_nn_params': {'NN__activation': ['relu'], 'NN__alpha': [1.0],
                               'NN__hidden_layer_sizes': [(32, 32)], 'NN__learning_rate_init': [0.01]}
        }
    dataset2_details = {
            'data': loader.DryBeansData(verbose=verbose, seed=seed),
            'name': 'DryBeansData',
            'readable_name': 'Dry Beans Data',
            'best_nn_params': {'NN__activation': ['relu'], 'NN__alpha': [1.0],
                               'NN__hidden_layer_sizes': [(32, 32)], 'NN__learning_rate_init': [0.01]}
    }
    if args.dataset1:
