...
class Adult(DataLoader):
    ...
    def _preprocess_data(self):
        ...
        # to_encode.remove(13)
        # vec_data = one_hot.fit_transform(df[to_encode]).toarray()
        # vec_data = pd.DataFrame(vec_data, columns=np.arange(vec_data.shape[1])+14)

        self._data = self._data.drop(to_encode, axis=1)
        self._data = pd.concat([self._data, df], axis=1)
        # self._data[13] = df[13]
        self._data.to_csv('data/adult_cleaned.csv', index=False)
...
class DryBeansData(DataLoader):
    def __init__(self, path='data/dry_beans_data.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
    ...
    def _preprocess_data(self):
        ...
        self._data = self._data.drop(to_encode, axis=1)
        self._data = pd.concat([self._data, df], axis=1)
        self._data.to_csv('data/dry_beans_data_cleaned.csv', index=False)
...
if __name__ == '__main__':
    cd_data = Adult(verbose=True)
    cd_data.load_and_process()

    ca_data = DryBeansData(verbose=True)
